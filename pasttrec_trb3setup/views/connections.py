...
def add_connection_view(request, rev=None, tdc=None):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('pasttrec_trb3setup:setup', instance.revision.setup.pk)
        else:
        ...
        return render(
            request,
            'pasttrec_trb3setup/card_connection.html',
            context = {
                'form' : form,
            }
        );
...
def insert_connection_view(request, rev=None, tdc=None):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('pasttrec_trb3setup:insert_connection', rev, tdc)
        ...
        return render(
            request,
            'pasttrec_trb3setup/insert_connection.html',
            context = {
                'form' : form,
                'revision' : revision,
                'tdc' : tdc,
...
def delete_connection_view(request, pk=None):
    if request.method == 'POST':
        ...
        return render(
            request,
            'pasttrec_trb3setup/remove_connection.html',
            context = {
                'connection' : instance,
            }
        );
