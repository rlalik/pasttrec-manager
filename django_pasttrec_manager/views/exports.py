...
def export_json_view(request, pk):
    ...
    # return redirect('django_pasttrec_manager:rev', pk=pk)
...
def import_select_view(request, setup):
    ...
    return render(
        request,
        "django_pasttrec_manager/revision_select.html",
        context={"setup": _setup, "object_list": _rev, "form": _form},
    )
...
def import_insert_view(request, setup):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("django_pasttrec_manager:import_file", rev=new_rev.pk)

    return redirect("django_pasttrec_manager:import_select_revision", setup=setup)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if res1 == False:
                return render(
                    request,
                    "django_pasttrec_manager/import_file.html",
                    context={
                        "setup": _rev.setup,
                        "rev": _rev,
                        "form_upload": form,
            ...
            return redirect("django_pasttrec_manager:import_file", rev=_rev.pk)
...
def import_json_view(request, rev):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for v in l:
                ...
                if v["sel"] is not "":
                    obj, created = CardSettings.objects.update_or_create(
                        card=Card.objects.get(pk=v["sel"]), revision=_rev
                    )
                    data = v["val"]
                    # asic #1
            ...
            return render(
                request,
                "django_pasttrec_manager/import_json.html",
                context={
                    "setup": _rev.setup,
                    "rev": _rev,
                    "updated": updated,
