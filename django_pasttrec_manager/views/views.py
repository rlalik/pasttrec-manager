...
def find_last_card_revision(card, rev):
    ...
    r = CardSettings.objects.filter(
        card=card.pk, revision__setup=rev.setup, revision__creation_on__lte=rev.creation_on
...
def create_revision_snapshot(revision, follow_maps=False):
    ...
    result = Connection.objects.filter(revision__setup=revision.setup, revision__creation_on__lte=revision.creation_on)
...
def create_revisions_map(setup):
    ...
    revs = Revision.objects.filter(setup=setup).order_by("creation_on")
    nrevs = len(revs)

    cons = Connection.objects.filter(revision__setup=setup)
    ncons = len(cons)
    ...
    for _r in range(nrevs):
        # get connetion
        cons = Connection.objects.filter(revision=revs[_r])
